	...
	struct cmdline_opt *optvals)
{
	...
	int ret, val;
	int output_flag;
	unsigned long i_start, i_end, d_end, s_end;
	...
	printf("insert time: %.6f sec\n", (double)(i_end - i_start) / 1E6);
	/*printf("delete: %.6f sec\n", (double)(d_end - i_end) / 1E6);*/
	printf("search time: %.6f sec\n", (double)(s_end - i_end) / 1E6);
	fclose(file);
}
